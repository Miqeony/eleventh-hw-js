...
const user = {
    ...
    addMoney(amount) {
        this.balance += amount;
        console.log(`Баланс ${this.name} збільшено на ${amount}.`);
    },

    balanceInfo() {
        console.log(`Баланс користувача ${this.name} становить: ${this.balance}`);
    },
};


const filteredUsers = users.filter(({ balance }) => balance > 420);
console.log(filteredUsers);


const balances = users.map(({ balance }) => balance);
console.log('Баланси всіх користувачів:', balances);

user.balanceInfo.call(filteredUsers[2]);
